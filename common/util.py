...
def cos_similarity(x, y, eps=1e-8):
    ...
    :param x: 벡터
    :param y: 벡터
    :param eps: '0으로 나누기'를 방지하기 위한작은 값
    :return:
    '''
...
def most_similar(query, word_to_id, id_to_word, word_matrix, top=5):
    ...
    :param id_to_word: 단어 ID에서 단어로 변환하는 딕셔너
...
def analogy(a, b, c, word_to_id, id_to_word, word_matrix, top=5, answer=None):
    ...
    smiliarity = np.dot(word_matrix, query_vec)
...
def create_context_target(corpus, window_size=1):
    ...
    :param window_size: 윈도우 크기(윈도우 크기가 1이면 타깃 단어 좌우 한 단어씩이 맥락에 포)
    ...
    for idx in range(window_size, len(corpus)-window_size):
        cs = []
        # window_size 만큼 타겟 단어 좌우 context 가져오기
        for t in range(-window_size, window_size+1):
            if t != 0:
                cs.append(corpus[idx + t])
        contexts.append(cs)
